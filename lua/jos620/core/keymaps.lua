...
local function setup_nvim_tree_keymaps(bufnr)
  ...
  set("n", "<C-]>", api.tree.change_root_to_node, "CD", opts)
  set("n", "<C-e>", api.node.open.replace_tree_buffer, "Open: In Place", opts)
  set("n", "<C-k>", api.node.show_info_popup, "Info", opts)
  set("n", "<C-r>", api.fs.rename_sub, "Rename: Omit Filename", opts)
  set("n", "<C-t>", api.node.open.tab, "Open: New Tab", opts)
  set("n", "<C-v>", api.node.open.vertical, "Open: Vertical Split", opts)
  set("n", "<C-x>", api.node.open.horizontal, "Open: Horizontal Split", opts)
  set("n", "<BS>", api.node.navigate.parent_close, "Close Directory", opts)
  set("n", "<CR>", api.node.open.edit, "Open", opts)
  set("n", "<Tab>", api.node.open.preview, "Open Preview", opts)
  set("n", ">", api.node.navigate.sibling.next, "Next Sibling", opts)
  set("n", "<", api.node.navigate.sibling.prev, "Previous Sibling", opts)
  ...
  set("n", "[c", api.node.navigate.git.prev, "Prev Git", opts)
  set("n", "]c", api.node.navigate.git.next, "Next Git", opts)
  set("n", "d", api.fs.remove, "Delete", opts)
  set("n", "D", api.fs.trash, "Trash", opts)
  set("n", "E", api.tree.expand_all, "Expand All", opts)
  set("n", "e", api.fs.rename_basename, "Rename: Basename", opts)
  ...
  set("n", "J", api.node.navigate.sibling.last, "Last Sibling", opts)
  set("n", "K", api.node.navigate.sibling.first, "First Sibling", opts)
  set("n", "m", api.marks.toggle, "Toggle Bookmark", opts)
  set("n", "o", api.node.open.edit, "Open", opts)
  set("n", "O", api.node.open.no_window_picker, "Open: No Window Picker", opts)
  set("n", "p", api.fs.paste, "Paste", opts)
  set("n", "P", api.node.navigate.parent, "Parent Directory", opts)
  ...
  set("n", "S", api.tree.search_node, "Search", opts)
  set("n", "U", api.tree.toggle_custom_filter, "Toggle Hidden", opts)
  set("n", "W", api.tree.collapse_all, "Collapse", opts)
  set("n", "x", api.fs.cut, "Cut", opts)
  set("n", "y", api.fs.copy.filename, "Copy Name", opts)
  set("n", "Y", api.fs.copy.relative_path, "Copy Relative Path", opts)
  set("n", "<2-LeftMouse>", api.node.open.edit, "Open", opts)
  set("n", "<2-RightMouse>", api.tree.change_root_to_node, "CD", opts)

  set("n", "e", "", { buffer = bufnr })
